...
def aircoBedroom(event):
    ...
    if tempBedroom > tempThreshold:
        ...
        turnOn = 1
        aircoBedroom.log.info("06. Bedroom temp [{}] > [{}], turn-AC-on flag is set to [{}]".format(tempBedroom, tempThreshold, turnOn))
    else:
        ...
        if tempOutside < outsideTempLoThreshold:
            ...
            turnOn = -1
            aircoBedroom.log.info("07. Bedroom temp is [{}], turn-AC-on flag set to [{}]".format(tempBedroom, turnOn))
        else:
            aircoBedroom.log.info("08. Bedroom temp [{}] below threshold, but outside temp [{}] above threshold, not turning off the AC if still on, AC status is [{}]".format(tempBedroom, tempOutside, items["AC_Power_Bedroom"] == ON))
    ...
    if doorIsOpen:
        if turnOn == 1:
            ...
            turnOn = -1
            aircoBedroom.log.info("09. The door is open and Turn-AC-On flag is set")
        if items["AC_Power_Bedroom"] == ON:
    ...
    if turnOn == 1:
        if items["AC_Power_Bedroom"] != ON:
            ...
            msg = ""
            aircoBedroom.log.info("12. AC already on, ignoring turn on action")
    elif turnOn == -1:
        if items["AC_Power_Bedroom"] == ON:
            ...
            msg = ""
            aircoBedroom.log.info("14. AC already off, ignoring turn off action")
    ...
    if msg != "":
        if (turnOn and items["AC_Power_Bedroom"] != ON) or (not turnOn and items["AC_Power_Bedroom"] == ON):    #acIsOn != turnOn:
            ...
            if items["AC_MessageRepeat"] != ON:
                aircoBedroom.log.info("15. Sending message [{}] to Alexa".format(msg))
                # Echo_TTS_Livingroom.sendCommand(msg)
                events.sendCommand("AC_MessageRepeat", "ON")
            else:
                aircoBedroom.log.info("16. Skipping message [{}] to Alexa".format(msg))
    else:
        aircoBedroom.log.info("17. Airco on state is [{}], desired turn on state [{}]".format(items["AC_Power_Bedroom"] == ON, turnOn))
