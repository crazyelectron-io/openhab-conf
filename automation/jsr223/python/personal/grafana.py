...
----------------------------------------------------------------------------------------------------
Changelog:
20200224 v05    Removed autogenerated metadata information from dictionary.
...
from configuration import LOG_PREFIX, GRAFANA_URL_BASE, grafImgDict
    ...
    try:
        for entry in grafImgDict.values():
            ...
            if ir.getItems(imgItemName) == []:
                add_item(imgItemName, item_type="Image", groups=[GRAFANA_GROUP], label=entry.get("item_description"), tags=["Grafana"])
...
def refreshImageItem(itemName):
    ...
    if rangeTo != "":
        panelUrl += "&to=" + rangeTo
    panelUrl += "&theme=" + str(get_key_value(itemName, "Grafana_Graphs", "Theme"))
